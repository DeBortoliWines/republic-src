...
* v0.6.7 - "Saved Multi Row" feature
...
| 5     | `<row-index>`         | Denotes the column index for the saved multi row parse type.                                                                                                                                                   |
...
| NewSavedMultiRowIf           | If matched, it saves the most recent lines of the next `<start-row>` to `<end-row>` then allow the user to select any field values in that buffer based on start/end column and a particular row. See SelectSavedMultiRowFieldData.                                                 |
| SelectSavedMultiRowFieldData | Selects all data between `<start-col>` and `<end-col>` from the last matched `NewSavedMultiRowIf`. You'll also need to specify the `<row-inde>` so republic knows which saved row to pull the data from.                                                                            |

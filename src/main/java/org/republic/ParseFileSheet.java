...
public class ParseFileSheet {
  ...
  public void parseInputFile(ParseSheet parseSheet, File inputReport) {
    ...
    try {
      ...
      while (reportLine != null) {
        ...
        iLineCtr++;
        logger.fine("line:" + iLineCtr + " , line length() " + reportLine.length());
        if (reportLine.length() >= 1) {
          int ascii = (int) reportLine.charAt(0);
          logger.fine(">>>>First char:" + ascii + " new page?: " + (ascii == 12));
          if (ascii == 12) {
            reportPagePos = 0;
            newPage = true;
        ...
        multiRowMode = checkReportLine(multiRowMode, parseSheet, reportLine, reportPagePos, in);
      ...
      if (rowExists()) writeRow();
    } catch (Exception e) {
      e.printStackTrace();
      logger.severe("Unable to Read Input File: " + inputReport + " Exception: " + e);
      return;
    }
  }
  ...
  public InputStream getLocalOrJarResource(String inputResource, String inputResourceLocation) {
    InputStream is = null;
    try {
      is = ParseFileSheet.class.getClassLoader().getResourceAsStream(inputResource);
    } catch (Exception e1) {
   ...
   * it..
   */
  public boolean checkReportLine(
      boolean multiRowMode,
    ...
    while (iter.hasNext()) {
      ...
      if (multiRowMode) {
        if (parseRule.getParseType().equalsIgnoreCase(CommonNames.ENDSHEETIF)) {
        ...
        if (parseRule.getParseType().equalsIgnoreCase(CommonNames.IGNOREPAGEROWS)) {
          if (reportPagePos >= parseRule.getStartRow() && reportPagePos <= parseRule.getEndRow()) {
            // Ignore this row. It is between the row points to
            // ignore.
            logger.fine("Ignore;IgnorePageRows:" + reportPagePos + "ReportLine:" + reportLine);
            return multiRowMode;
          }
        }

        if (parseRule.getParseType().equalsIgnoreCase(CommonNames.IGNORENEWPAGEUNTIL)
            && newPage == true) {
          ...
          if (ruleMatchesReportLine(parseRule, reportLine)) {
            ...
            logger.fine(
                CommonNames.IGNORENEWPAGEUNTIL + reportPagePos + "ReportLine:" + reportLine);
            return multiRowMode;
          }
        } else {
          ...
          logger.fine("MultiRow to Buffer:" + getMultiRowLine() + reportLine);
          ...
          if (getMultiRowLine() >= getMultiRowMaxLines()) {
            logger.fine(
                "About to process the buffer, lines:"
                    + getMultiRowLine()
                    + " max lines >= "
                    + getMultiRowMaxLines());
        ...
        if (parseRule.getParseType().equalsIgnoreCase(CommonNames.ENDSHEETIF)) {
        ...
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.IGNORELINEIF)) {
          logger.fine(CommonNames.IGNORELINEIF);
          if (ruleMatchesReportLine(parseRule, reportLine)) {
            logger.fine(CommonNames.IGNORELINEIF + " ignoring..!" + reportLine);
            return multiRowMode;
          }

        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.IGNOREPAGEROWS)) {
          if (reportPagePos >= parseRule.getStartRow() && reportPagePos <= parseRule.getEndRow()) {
            // Ignore this row. It is between the row points to
            // ignore.
            logger.fine("Ignore;IgnorePageRows:" + reportPagePos + "ReportLine:" + reportLine);
        ...
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.IGNORENEWPAGEUNTIL)
            && newPage == true) {
          // Look for this string match.. If found we are now off
          // ignore mode.
          if (ruleMatchesReportLine(parseRule, reportLine)) {
        ...
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.ALLDATAIF)) {
          if (ruleMatchesReportLine(parseRule, reportLine)) {
            allDataMode = true;
          }
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.NEWROWIF)) {
          ...
          try {
            ...
            if (ruleMatchesReportLine(parseRule, reportLine)) {
              if (rowExists()) writeRow();
              startNewRow();
            } else return multiRowMode;

          } catch (Exception e) {
            e.printStackTrace();
            logger.severe("Exception Parsing IgnoreLineIf: " + parseRule.getMatchString() + e);
          }

        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.NEWSAVEDROWIF)) {
          ...
          try {
            ...
            e.printStackTrace();
            logger.severe("Exception Parsing NewSavedRowIf: " + parseRule.getMatchString() + e);
          }

        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTFIELDDATA)) {
        ...
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTSAVEDROWFIELDDATA)) {
        ...
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTMULTIROWFIELDDATA)) {
          logger.fine(CommonNames.SELECTMULTIROWFIELDDATA + " ignoring rule!");
          // ignore except when buffering multi-row..
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.NEWMULTIROWIF)) {
          if (ruleMatchesReportLine(parseRule, reportLine)) {
            logger.fine("newMultiRowif triggered for line: " + reportLine);
            ...
            logger.info("MultiRowMaxLines:" + parseRule.getEndRow());
            setMultiRowMaxLines(parseRule.getEndRow());
            return true;
          }
        } else if (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTALLDATA)
            && allDataMode) {
  ...
  public void marshallParsedResults(String outputFile) {
    ...
    try {
      ...
      mapping.loadMapping(
          new InputSource(
              getLocalOrJarResource(
                  CommonNames.OUTPUTDATAMAPPING, CommonNames.OUTPUTDATAMAPPINGLOCATION)));
  ...
  public String getReportCompareString(String reportLine, int startCol, int endCol) {
    if (reportLine.length() < endCol) return "";
  ...
  public void transformToContent(InputStream xslFile, String inputXMLFile, String outputFileName) {
    try {
  ...
  public void transformToContent(
      StreamSource streamSource, String inputXMLFile, String outputFileName) {
    try {
      // StreamSource streamSource = new StreamSource(xslFile);
  ...
  public void transformToContent(String xslFile, String inputXMLFile, String outputFileName) {
    try {
  ...
  public void setColumnNames(ParseSheet parseSheet, OutputDataBean odb) {
    ...
    while (iter.hasNext()) {
      ParseRule parseRule = (ParseRule) iter.next();
      logger.fine(
          "Column Names, " + parseRule.getParseType() + " ParseType:" + parseRule.getParseType());

      if ((parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTFIELDDATA))
          || (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTMULTIROWFIELDDATA))
          || (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTSAVEDROWFIELDDATA))
          || (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTALLDATA))) {
  ...
  public boolean ruleMatchesReportLine(ParseRule parseRule, String reportLine) {
    if (parseRule.getStartCol() == 0 || parseRule.getEndCol() == 0) {
      logger.severe("Rule Requiring Match has NO Start or End Column defined!");
      return false;
    }
    String reportCompareString =
        getReportCompareString(reportLine, parseRule.getStartCol(), parseRule.getEndCol());
    ...
    if (parseRule.getMatchString().equalsIgnoreCase(CommonNames.BLANK)) {
      ...
      if (reportCompareString.equalsIgnoreCase(compareBlank)) return true;
    } else if (parseRule.getMatchString().equalsIgnoreCase(CommonNames.NOTBLANK)) {
      String compareBlank = fillString(reportCompareString.length(), " ");
      logger.fine("comparing: '" + reportCompareString + "' with '" + compareBlank + "'");
      logger.fine("not blank: " + (!reportCompareString.equalsIgnoreCase(compareBlank)));
      logger.info("not blank Len: '" + reportCompareString.length() + "'");
      if (reportCompareString.length() == 0) return false;
      if (!reportCompareString.equalsIgnoreCase(compareBlank)) return true;

    } else if (reportCompareString.equalsIgnoreCase(parseRule.getMatchString())) return true;
  ...
  /**
   * @param multiRowLine The multiRowLine to set.
   */
  ...
  /**
   * @param multiRowMaxLines The multiRowMaxLines to set.
   */
  ...
  public void loadMultiRows(ParseSheet parseSheet) {
    ...
    for (int lineCtr = 1; lineCtr < getMultiRowLine(); lineCtr++) {
      logger.fine("line: " + lineCtr + " val: " + multiRowReportLine[lineCtr]);
    }

    Iterator<ParseRule> iter = parseSheet.getParseRules().iterator();
    while (iter.hasNext()) {
      ParseRule parseRule = (ParseRule) iter.next();
      if (parseRule.getParseType().equalsIgnoreCase(CommonNames.SELECTMULTIROWFIELDDATA))
        try {
          ...
          if (parseRule.getStartRow() <= getMultiRowLine()) {
            logger.fine(
                ">MultiLineExtract row="
                    + parseRule.getStartRow()
                    + " startCol:"
                    + parseRule.getStartCol()
                    + " endCol:"
                    + parseRule.getEndCol());
            String reportLine = multiRowReportLine[parseRule.getStartRow()];
            logger.fine(">Multi2: " + reportLine);
            extractSubstringFromReportLine(
                reportLine, parseRule.getStartCol(), parseRule.getEndCol());
            String newValue =
                extractSubstringFromReportLine(
                    reportLine, parseRule.getStartCol(), parseRule.getEndCol());
            outputDataCol.setColumnValue(newValue);

            logger.fine("Value: " + reportLine + outputDataCol.getColumnValue());
  ...
  public String extractSubstringFromReportLine(String reportLine, int startCol, int endCol) {
    if (startCol == 0) return "";
    if (reportLine.length() >= endCol) return reportLine.substring(startCol - 1, endCol);
    else if (reportLine.length() >= (startCol - 1))
      return reportLine.substring(startCol - 1, reportLine.length());
    else return "";
  }

  public OutputDataColumn makeNewDataColumn(ParseRule parseRule, String reportLine) {
    OutputDataColumn outputDataCol = new OutputDataColumn();
    // Only extract if there is enough line left!
    logger.fine("Output Data: " + parseRule.getStartCol() + " end: " + parseRule.getEndCol());
    String newValue =
        extractSubstringFromReportLine(reportLine, parseRule.getStartCol(), parseRule.getEndCol());
    outputDataCol.setColumnValue(newValue);
    ...
    logger.fine(
        "Report Line: " + reportLine + ", Value Extracted: " + outputDataCol.getColumnValue());
    outputDataCol.setColumnFieldType(parseRule.getOutputFieldType());
  ...
  /**
   * @param parseSheetColumnNames The parseSheetColumnNames to set.
   */
  ...
  /**
   * @param sheetOutputDataColumns The sheetOutputDataColumns to set.
   */
  ...
  /**
   * @param parseSheetOutputDataRows The parseSheetOutputDataRows to set.
   */
